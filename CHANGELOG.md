...
- **Tags: named model suites as reusable presets.** `fastevals tag
  ...
  `fastevals --tag NAME`, the MCP `run_evaluation(tag=...)` argument, and
  `RunConfig(tag=...)` in Python all run a suite without retyping models.
  New MCP tools `add_tag` and `list_tags` let agents define and discover
  suites themselves; invalid selectors are rejected at save time.
- **Four adaptive built-in tags**: `auto-fast` (lightest cell per model),
  `auto-deep` (deepest per model), `auto-cheap` (cheapest model, lightest
  effort) and `auto-flagship` (priciest model, all efforts) — computed from
  the current registry on every use, so they work on day zero and never go
  stale. The names are reserved against user overrides.
